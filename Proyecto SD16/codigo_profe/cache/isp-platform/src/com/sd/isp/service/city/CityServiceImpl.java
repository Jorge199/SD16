...
@Service
@Transactional
public class CityServiceImpl extends BaseServiceImpl<CityDTO, CityDomain, CityDaoImpl, CityResult>
	...
	@Override
	@Transactional
	@Cacheable(value = "isp-platform-cache", key = "'city_' + #id")
	...
	protected CityDTO convertDomainToDto(CityDomain domain) {
		...
		dto.setCountryId(domain.getState().getCountry().getId());
		dto.setStateId(domain.getState().getId());
	...
	protected CityDomain convertDtoToDomain(CityDTO dto) {
		...
		if (null != dto.getStateId()) {
			domain.setState(stateDao.getById(dto.getStateId()));
		}
