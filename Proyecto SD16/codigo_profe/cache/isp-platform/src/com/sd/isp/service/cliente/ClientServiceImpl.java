...
@Service
public class ClientServiceImpl extends BaseServiceImpl<ClientDTO, ClientDomain, ClientDaoImpl, ClientResult>
	...
	@Override
	@Transactional
	@CacheEvict(value = "isp-platform-cache", key = "'client_' + #client.id", condition="#dto.id!=null")
	//@CachePut(value = "isp-platform-cache", key = "'client_' + #client.id", condition="#dto.id!=null")
	public ClientDTO save(ClientDTO dto) {
		final ClientDomain clientDomain = convertDtoToDomain(dto);
		final ClientDomain client = clientDao.save(clientDomain);
		final ClientDTO newDto = convertDomainToDto(client);
		if (dto.getId() == null) {
			getCacheManager().getCache("isp-platform-cache").put("client_" + client.getId(), newDto);
		}
		return newDto;
	}
