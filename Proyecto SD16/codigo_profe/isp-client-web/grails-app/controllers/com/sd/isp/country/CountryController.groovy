...
class CountryController {
	...
	def list(Integer max) {
		def countries = countryService.getAll()
		[countryInstanceList: countries, countryInstanceTotal: countries.size()]
	}

	def create() {
		[countryInstance: new CountryB(params)]
	}

	def save() {
		def countryInstance = new CountryB(params)
		def newCountry = countryService.save(countryInstance)
		if (!newCountry?.getId()) {
		...
		redirect(action: "show", id: newCountry.getId())
	}

	def show(Long id) {
		def countryInstance = countryService.getById(id.intValue())
		if (!countryInstance) {
